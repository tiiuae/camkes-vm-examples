...
static vm_frame_t dataport_memory_iterator(uintptr_t addr, void *cookie)
{
    ...
    frame_result.cptr = dataport_get_nth_frame_cap(&buff_handle, page_idx);
...
void init_ram_module(vm_t *vm, void *cookie)
{
    ...
    if (linux_ram_base == 0x40000000) {
	ZF_LOGI("initializing RAM module the old way");
        do_init_ram_module(vm, cookie); //vm0 
        printf("This is (init OLD): %s \n", linux_image_config.vm_name);
        return;
    }

    ZF_LOGI("initializing RAM module from dataport");
    printf("This is (init CUSTOM): %s \n", linux_image_config.vm_name);
    err = vm_ram_register_at_custom_iterator(vm, linux_ram_base, linux_ram_size, dataport_memory_iterator, NULL); //vm1
    assert(!err);
}
